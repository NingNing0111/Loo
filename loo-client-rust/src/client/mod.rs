...
impl ClientApp {
    ...
    async fn connect(&mut self) -> Result<()> {
        ...
        let server_channel = TcpStream::connect(&server_addr)
            .await
            .with_context(|| format!("无法连接到服务器: {}", server_addr))?; // 更详细的错误信息
    ...
    async fn handler(&mut self) -> Result<()> {
        let auth_message = build_auth_message(self.config.get_password());
